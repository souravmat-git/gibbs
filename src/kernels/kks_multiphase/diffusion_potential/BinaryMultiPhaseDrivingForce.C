...
registerMooseObject("gibbsApp", BinaryMultiPhaseDrivingForce);

template <>
InputParameters
validParams<BinaryMultiPhaseDrivingForce>()
{
...
BinaryMultiPhaseDrivingForce::BinaryMultiPhaseDrivingForce(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
BinaryMultiPhaseDrivingForce::computeQpResidual()
{
  
  return (_L[_qp]*(_test[_i][_qp] * _nd_factor[_qp] 
          * _dh[_qp] * (_A_chem_pot_2[_qp] -_A_chem_pot_1[_qp])));
}   
    
Real
BinaryMultiPhaseDrivingForce::computeQpJacobian()
{
   return (_L[_qp]*(_test[_i][_qp] * _nd_factor[_qp] * _d2h[_qp] * (_A_chem_pot_2[_qp] 
                                                -   _A_chem_pot_1[_qp])* _phi[_j][_qp]));
}

Real
BinaryMultiPhaseDrivingForce::computeQpOffDiagJacobian( unsigned int jvar)
{
    
  if (jvar == _B_diff_pot_var)
  {
    return  (_L[_qp]*(_test[_i][_qp] * _nd_factor[_qp] * _dh[_qp] * (_xB_1[_qp] - _xB_2[_qp])* _phi[_j][_qp])); 
  }
  else if (jvar == _phase_2_var)
  {
    return (_L[_qp]*(_test[_i][_qp] * _nd_factor[_qp] * _d2h_2[_qp]* (_A_chem_pot_2[_qp] 
                                                   - _A_chem_pot_1[_qp])* _phi[_j][_qp]));
  }
  else if (jvar == _phase_3_var)
  {
    return (_L[_qp]*(_test[_i][_qp] * _nd_factor[_qp] * _d2h_3[_qp]* (_A_chem_pot_2[_qp] 
                                                   - _A_chem_pot_1[_qp])* _phi[_j][_qp]));
  }
  else if (jvar == _phase_4_var)
  {
    return (_L[_qp]*(_test[_i][_qp] * _nd_factor[_qp] * _d2h_4[_qp]* (_A_chem_pot_2[_qp] 
                                                   - _A_chem_pot_1[_qp])* _phi[_j][_qp]));
  }
  else if (jvar == _phase_5_var)
  {
    return (_L[_qp]*(_test[_i][_qp] * _nd_factor[_qp] * _d2h_5[_qp]* (_A_chem_pot_2[_qp] 
                                                   - _A_chem_pot_1[_qp])* _phi[_j][_qp]));
  }
  else    
      return 0.0; 
