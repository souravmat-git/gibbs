...
#include "KKS3PhaseEqualGPBetaGamma.h"

registerMooseObject("gibbsApp", KKS3PhaseEqualGPBetaGamma);

template <>
InputParameters
validParams<KKS3PhaseEqualGPBetaGamma>()
{
  InputParameters params = validParams<Kernel>();
  params.addClassDescription("Eqn: df/dphi_{gamma} = dh_{gamma}/dphi{_gamma}*(omega_beta- omega_gamma)."
                             "omega_beta, omega_gamma is the grand potential"
                             "This kernel operates on phi_gamma.");
  ...
  // This varible may not be required always
  // Can be used to model two phase problem
...
KKS3PhaseEqualGPBetaGamma::KKS3PhaseEqualGPBetaGamma(const InputParameters & parameters)
  : Kernel(parameters),
    ...
    _d2hbeta_dphigamma2(getMaterialProperty<Real>("d2hbeta_dphigamma2")),
...
Real
KKS3PhaseEqualGPBetaGamma::computeQpResidual()
{
  ...
  return _test[_i][_qp] * _L[_qp] * _dhbeta_dphigamma[_qp] * (omega_beta - omega_gamma);
}   
    
Real
KKS3PhaseEqualGPBetaGamma::computeQpJacobian()
{
  ...
  return _test[_i][_qp] *_L[_qp] * _d2hbeta_dphigamma2[_qp] * (omega_beta - omega_gamma) * _phi[_j][_qp];
}

Real
KKS3PhaseEqualGPBetaGamma::computeQpOffDiagJacobian( unsigned int jvar)
{
  ...
  if (jvar == _phase_comp_beta_var)
  {
    ...
    return  - _test[_i][_qp] * _L[_qp] * _dhbeta_dphigamma[_qp] * 
  ...
  {
    return  _test[_i][_qp] * _L[_qp] * _d2hbeta_dphigamma_dphialpha[_qp] * 
                                  (omega_beta - omega_gamma) * _phi[_j][_qp];
  }
  else if (jvar == _phase_beta_var)
  {
    return _test[_i][_qp] * _L[_qp] * _d2hbeta_dphigamma_dphibeta[_qp] *
                                  (omega_beta - omega_gamma) * _phi[_j][_qp];                                  
  }
