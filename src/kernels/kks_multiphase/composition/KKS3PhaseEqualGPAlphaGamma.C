...
#include "KKS3PhaseEqualGPAlphaGamma.h"

registerMooseObject("gibbsApp",KKS3PhaseEqualGPAlphaGamma);

template <>
InputParameters
validParams<KKS3PhaseEqualGPAlphaGamma>()
{
  InputParameters params = validParams<Kernel>();
  params.addClassDescription("Eqn: df/dphi_{gamma} = dh_{alpha}/dphi{_gamma}*(omega_alpha - omega_gamma)."
...
KKS3PhaseEqualGPAlphaGamma::KKS3PhaseEqualGPAlphaGamma(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
KKS3PhaseEqualGPAlphaGamma::computeQpResidual()
{
  ...
  return _test[_i][_qp] * _L[_qp] * _dhalpha_dphigamma[_qp] * (omega_alpha - omega_gamma);
}   
    
Real
KKS3PhaseEqualGPAlphaGamma::computeQpJacobian()
{
  ...
  return _test[_i][_qp] *_L[_qp] * _d2halpha_dphigamma2[_qp] * (omega_alpha - omega_gamma) * _phi[_j][_qp];
}

Real
KKS3PhaseEqualGPAlphaGamma::computeQpOffDiagJacobian( unsigned int jvar)
{
  ...
  if (jvar == _phase_comp_gamma_var)
  {
    ...
    return   _test[_i][_qp] * _L[_qp] * _dhalpha_dphigamma[_qp] * 
  ...
  {
    return  _test[_i][_qp] * _L[_qp] * _d2halpha_dphigamma_dphibeta[_qp] * 
                                  (omega_alpha - omega_gamma) * _phi[_j][_qp];
  }
  else if (jvar == _phase_alpha_var)
  {
    return _test[_i][_qp] * _L[_qp] * _d2halpha_dphigamma_dphialpha[_qp] *
                                  (omega_alpha - omega_gamma) * _phi[_j][_qp];                                  
  }
