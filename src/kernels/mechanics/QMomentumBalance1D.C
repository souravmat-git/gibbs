...
#include "QMomentumBalance1D.h"
registerMooseObject("gibbsApp", QMomentumBalance1D);

template <>
InputParameters
validParams<QMomentumBalance1D>(){
...
QMomentumBalance1D::QMomentumBalance1D(const InputParameters & parameters)
  :Kernel(parameters),
  ...
  _da_dh(getMaterialProperty<Real>(getParam<MaterialPropertyName>("da_dh"))),
...
Real
QMomentumBalance1D::interp_modulus() const{
  return (_h[_qp]* _mat_const_beta[_qp] + (1.0-_h[_qp]) * _mat_const_alpha[_qp]);
}


Real
QMomentumBalance1D::computeQpResidual()
...
Real
QMomentumBalance1D::computeQpJacobian()
{  //The derivative w.r.t displacement field
  return -_grad_test[_i][_qp](0) * (QMomentumBalance1D::interp_modulus() 
         + _da_de[_qp]*_h[_qp]*(1-_h[_qp])*(_mat_const_beta[_qp] - _mat_const_alpha[_qp]))*_grad_phi[_j][_qp](0);
}     

Real
QMomentumBalance1D::computeQpOffDiagJacobian(unsigned int jvar)
{
 if (jvar == _eta_var)
 { //Derivative w.r.t phi
  return -_grad_test[_i][_qp](0) * _dh[_qp] * ( (_sx_beta[_qp]- _sx_alpha[_qp]) -_a[_qp]* QMomentumBalance1D::interp_modulus()
