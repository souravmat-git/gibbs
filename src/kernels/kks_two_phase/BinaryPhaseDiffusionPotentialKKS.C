...
registerMooseObject("gibbsApp", BinaryPhaseDiffusionPotentialKKS);

template <>
InputParameters
validParams<BinaryPhaseDiffusionPotentialKKS>()
{
...
BinaryPhaseDiffusionPotentialKKS::BinaryPhaseDiffusionPotentialKKS(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
BinaryPhaseDiffusionPotentialKKS::computeQpResidual()
{
  return (_test[_i][_qp] * (_B_diff_pot_beta[_qp] - _B_diff_pot_alpha[_qp]));
}

Real
BinaryPhaseDiffusionPotentialKKS::computeQpJacobian()
{
...
Real
BinaryPhaseDiffusionPotentialKKS::computeQpOffDiagJacobian(unsigned int jvar)
{
