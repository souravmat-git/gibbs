...
#include "PlaneStressIsotropicModulus.h"
registerMooseObject("gibbsApp", PlaneStressIsotropicModulus);

//template <>
InputParameters
PlaneStressIsotropicModulus::validParams()
{
...
PlaneStressIsotropicModulus::PlaneStressIsotropicModulus(const InputParameters & parameters)
  : Material(parameters),
...
void
PlaneStressIsotropicModulus::computeQpProperties()
{

  _C11_val[_qp] = _E[_qp]/(1.0 - _nu[_qp] * _nu[_qp]);
  _C22_val[_qp] = _E[_qp]/(1.0 - _nu[_qp] * _nu[_qp]) ;
  _C12_val[_qp] = _nu[_qp]*_C22_val[_qp];
