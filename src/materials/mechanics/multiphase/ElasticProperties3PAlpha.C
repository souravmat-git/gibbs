...
#include "ElasticProperties3PAlpha.h"
registerMooseObject("gibbsApp", ElasticProperties3PAlpha);

template <>
InputParameters
validParams<ElasticProperties3PAlpha>()
{
...
ElasticProperties3PAlpha::ElasticProperties3PAlpha(const InputParameters & parameters)
  : Material(parameters),
...
void
ElasticProperties3PAlpha::computeQpProperties()
{
   _elastic_strain_val[_qp] = _total_strain[_qp](0,0)
                              + (_h_beta[_qp] + _h_gamma[_qp]) * _a_alpha_beta[_qp]
