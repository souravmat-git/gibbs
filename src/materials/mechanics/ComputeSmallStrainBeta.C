...
#include "ComputeSmallStrainBeta.h"
registerMooseObject("gibbsApp", ComputeSmallStrainBeta);

//template <>
InputParameters
ComputeSmallStrainBeta::validParams()
{
...
ComputeSmallStrainBeta::ComputeSmallStrainBeta(const InputParameters & parameters)
...
void
ComputeSmallStrainBeta::computeQpProperties()
{
  //Define the displacement gradient tensor which is in general not symmetric
  const auto _disp_grad_tensor =  RankTwoTensor::initializeFromRows
                                  ((*_grad_disp[0])[_qp],
                                    (*_grad_disp[1])[_qp],
                                    (*_grad_disp[2])[_qp]);
  ...
  if (_global_strain)
   ...
   //elastic strain is modified
   _mechanical_strain[_qp] -= (1.0- _h[_qp]) * _strain_jump[_qp] ;
}
