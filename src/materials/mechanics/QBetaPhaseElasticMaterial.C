...
#include "QBetaPhaseElasticMaterial.h"
registerMooseObject("gibbsApp", QBetaPhaseElasticMaterial);

//template <>
InputParameters
QBetaPhaseElasticMaterial::validParams()
{
...
QBetaPhaseElasticMaterial::QBetaPhaseElasticMaterial(const InputParameters & parameters)
  : Material(parameters),
...
void
QBetaPhaseElasticMaterial::computeQpProperties(){

  _elastic_strain_val[_qp].zero();
  _stress_val[_qp].zero();

  //(total) compatible strain - beta_eigenstrain
  //-  strain jump * (1-interpolation)
   _elastic_strain_val[_qp] = _total_strain[_qp]
                    - (1.0- _h[_qp]) * _strain_jump[_qp] - _beta_eigen[_qp];

  //Following this calculate the phase stress
   _stress_val[_qp] = _beta_stiffness[_qp]* _elastic_strain_val[_qp];

  //J_ijpq = C_ijpq - (1-h)*C_ijkl * ds_kl/de_pq, where ds_kl/de_pq = M_klpq
  //_Jacobian_mult[_qp] = _beta_stiffness[_qp]
  //                      - (1.0 -_h[_qp])* _beta_stiffness[_qp] * _ds_de[_qp];
