...
#include "DerivativeStrainJumpMaterialN.h"
registerMooseObject("gibbsApp", DerivativeStrainJumpMaterialN);

//template <>
InputParameters
DerivativeStrainJumpMaterialN::validParams()
{
...
DerivativeStrainJumpMaterialN::DerivativeStrainJumpMaterialN(const InputParameters & parameters)
  : Material(parameters),
   ...
   //Inverse of K tensor and its derivatives
...
void
DerivativeStrainJumpMaterialN::computeQpProperties()
{
  ...
  if (_norm_sq < std::pow(libMesh::TOLERANCE, 3.0)){
        ...
        for (unsigned int q = 0; q <LIBMESH_DIM; q++)
          for (unsigned int i = 0; i <LIBMESH_DIM; i++)
            for (unsigned int k = 0; k < LIBMESH_DIM; k++){
              _da_de(l,p,q) += - _inv_K[_qp](l,i)*(_stiffness_alpha[_qp](p,q,k,i)
    ...
    /************  Calculate da/dphi ***************************/
     _da_dphi_val[_qp] = -_inv_K[_qp] * _dK_dh[_qp] *_a[_qp] *_dh[_qp];
  }
}
