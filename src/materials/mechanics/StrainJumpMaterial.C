...
#include "StrainJumpMaterial.h"

registerMooseObject("gibbsApp", StrainJumpMaterial);

template <>
InputParameters
validParams<StrainJumpMaterial>()
{
...
StrainJumpMaterial::StrainJumpMaterial(const InputParameters & parameters)
  : Material(parameters),
...
void
StrainJumpMaterial::computeQpProperties()
{//return the strain jump and its derivative wrt strain 
  _a_val[_qp] =    -(_mat_const_beta[_qp]*(_grad_ux[_qp](0) - _eT_beta[_qp]) - _mat_const_alpha[_qp] * (_grad_ux[_qp](0) - _eT_alpha[_qp]))/
                        (_mat_const_beta[_qp]* (1.0 - _h[_qp]) + _mat_const_alpha[_qp]*_h[_qp]);
                        
  _da_de_val[_qp] = -(_mat_const_beta[_qp] - _mat_const_alpha[_qp])/
                        (_mat_const_beta[_qp]* (1.0 - _h[_qp]) + _mat_const_alpha[_qp]*_h[_qp]);
                        
 _da_dh_val[_qp] =  _a_val[_qp]*(_mat_const_beta[_qp] - _mat_const_alpha[_qp])/
                        (_mat_const_beta[_qp]* (1.0 - _h[_qp]) + _mat_const_alpha[_qp]*_h[_qp]);          
}
